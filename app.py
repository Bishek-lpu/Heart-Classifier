...
def predict_clinical():
    try:
        ...
        chol_level = pd.cut([chol], bins=[0, 200, 240, 600], labels=[0, 1, 2]).astype(
            int
        )[0]
        thalach_level = pd.cut(
            [thalach], bins=[0, 100, 150, 220], labels=[0, 1, 2]
        ).astype(int)[0]
        ...
        factors = contributing_factors(
            age, sex, trestbps, chol, thalach, exang, oldpeak, cp
        )
        ...
        suggestion = get_ai_suggestion(
            "Clinical Data", input_df.values.tolist(), result_snapshot
        )
...
def get_report():
    try:
        ...
        pdf_bytes = generate_report_pdf(
            title, mode_label, input_snapshot, result_snapshot, suggestion, contact_info
        )
